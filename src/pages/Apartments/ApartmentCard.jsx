  ...
  const handleAgreement = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure.post("/add-agreement", data).then((res) => {
          refetch();
          console.log(res);
          if (res.data.insertedId) {
            toast.success(
              "Agreement Request Sent Successfully. Wait for Admin's Approval."
            );
          } else {
            toast.warning(
              "You may be already a member or requested for an apartment. Requesting for more than one apartment is not allowed."
            );
          }
        });
      }
